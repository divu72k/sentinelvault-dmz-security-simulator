...
import { Lock, Key, Eye, EyeOff, RefreshCcw, Binary, Shield } from 'lucide-react';
...
export default function EncryptionPage() {
  ...
  const handleGenerateKey = async () => {
    try {
      ...
      setKeyHex(hex);
      toast.success("New 256-bit AES-GCM Key generated");
    } catch (err) {
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input value={keyHex} readOnly placeholder="0x000..." className="font-mono text-xs bg-black/40" />
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className={cn("font-mono text-sm bg-black/40 min-h-[120px]", !showPlaintext && "blur-sm select-none")}
                  />
                </div>
                <Button className="w-full font-bold uppercase tracking-widest" disabled={!encryptionKey || isProcessing} onClick={handleEncrypt}>
            ...
            <Card className="h-full border-border/50 bg-secondary/20 backdrop-blur-sm border-dashed">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Input value={ivHex} readOnly className="font-mono text-xs bg-black/40 border-emerald-500/20" />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs uppercase font-bold text-muted-foreground tracking-widest">Ciphertext (Base16)</Label>
                  <div className="relative">
                    <Textarea value={ciphertextHex} readOnly className="font-mono text-xs bg-black/40 border-emerald-500/20 min-h-[200px]" />
                    {!ciphertextHex && (
